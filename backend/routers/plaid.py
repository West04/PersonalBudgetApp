...
# ... (Plaid client setup code is the same) ...
...
def create_link_token():
    ...
    This token is used by the frontend to initialize the Plaid Link modal.
    """
    try:
...
def exchange_public_token(
        payload: schemas.PlaidPublicTokenRequest,
    ...
    try:
        ...
        db_item = crud_plaid.create_plaid_item(
            db=db,
        ...
        for account in accounts_response.accounts:
            db_account = crud_plaid.create_account(
                db=db,
                account=account.to_dict(),
                item_id=db_item.id
            )
            saved_accounts.append(db_account)
...
class PlaidSyncRequest(schemas.BaseModel):
    plaid_item_id: str
...
def sync_transactions(
    ...
    """
    Fetches transaction updates from Plaid and syncs them
